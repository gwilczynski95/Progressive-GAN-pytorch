...
def imagefolder_loader(path, dataset):
    if dataset == 'mnist':
        loaded_data = datasets.MNIST(path, download=True)
        return loaded_data.train_data.numpy()
    elif dataset == 'cifar10':
        loaded_data = datasets.CIFAR10(path, download=True)
        return loaded_data.data


def get_data(path_to_data, no_of_data, dataset='mnist'):
    data = imagefolder_loader(path_to_data, dataset)
    rng = default_rng()
...
if __name__ == '__main__':
    ...
    path_to_training_info = '../trial_cifar_test_1_2021-09-26_13_26'
    ...
    original_data = get_data('/home/grzegorz/grzegos_world/13_september_2021/cifar', num_of_data_samples_for_fid,
                                   'cifar10')
    ...
    for generator_path in generators_paths:
        ...
        with torch.no_grad():
            ...
            for idx in range(num_of_data_samples_for_fid // gen_batch_size):
                ...
                gen_data = gen_data.squeeze(1).cpu().numpy()
