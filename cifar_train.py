...
def train(
    ...
    if continue_training:
        ...
        # prepare dataset
        data_loader = cifar_sample_data(loader, 4 * 2 ** step)
    ...
    for i in pbar:
        ...
        if (i + 1) % 1000 == 0 or i == 0:
            with torch.no_grad():
                images = g_running(torch.randn(5 * 10, config['generator']['input_code_dim']).to(config['device']),
...
def prepare_training(resume_path=None):
    ...
    generator = Generator(**config['generator']).to(config['device'])
    discriminator = Discriminator(**config['discriminator']).to(config['device'])
    g_running = Generator(**config['generator']).to(config['device'])
    ...
    grzegos_data_path = '/home/grzegorz/grzegos_world/13_september_2021/cifar/'
...
if __name__ == '__main__':
    # path_to_continue_training = '/home/grzegorz/grzegos_world/13_september_2021/Progressive-GAN-pytorch/'
